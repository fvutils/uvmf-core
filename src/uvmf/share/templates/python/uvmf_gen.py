...
class BaseElementClass(object):
  ...
  def addHvlParamDef(self,name,type,value=None):
    """Add an HVL parameter to the package"""
    self.hvlParamDefs.append(ParamDef(name,type,value))
...
class NonUvmfComponentClass(BaseElementClass):
  def __init__(self,name,type,parametersDict,instanceArraySize=''):
    super(NonUvmfComponentClass,self).__init__(name)
    self.type = type
    self.instanceArraySize = instanceArraySize
    self.parameters = []
...
class AgentClass(BaseElementClass):
  def __init__(self,name,ifPkg,clk,rst,agentIndex,parametersDict,initResp='INITIATOR',instanceArraySize=''):
...
class analysisComponentClass(BaseElementClass):
  def __init__(self,keyword,name,aeDict,apDict,qvipAeDict,parametersList,hvlParametersList,mtlbReady=False):
    ...
    for parameter in parametersList:
      try:
        ...
        pass
    for parameter in hvlParametersList:
      try:
        self.hvl_parameters.append(ParamDef(parameter['name'],parameter['type'],parameter['value']))
      except KeyError:
        self.hvl_parameters.append(ParamDef(parameter['name'],parameter['type'],None))
        pass

class BfmClass(BaseElementClass):
  def __init__(self,name,ifPkg,clk,rst,activity,parametersDict,sub_env_path,initResp,agentInstName,inFactReady,portsList,instanceArraySize,flat_bfm_name,is_arrayed,real_path):
    ...
    for parameterName in parametersDict:
      self.parameters.append(ParameterValueClass(parameterName,parametersDict[parameterName]))
    self.flat_bfm_name = flat_bfm_name
...
class SubEnvironmentClass(BaseElementClass):
  def __init__(self,name,envPkg,numAgents,agent_index,parametersDict,regModelPkg,regBlockClass,regBlockInstance,instanceArraySize=''):
...
class QvipSubEnvironmentClass(BaseElementClass):
  def __init__(self,name,envPkg,numAgents,agent_index,agentList,envHasICVIP,envHasQVIP,instanceArraySize=''):
...
class AnalysisExportClass(BaseElementClass):
  def __init__(self,name,tType,connection="",portName="",QVIPConn=False,instanceArraySize=None):
...
class AnalysisPortClass(BaseElementClass):
  def __init__(self,name,tType,connection="",portName="",instanceArraySize=None):
...
class analysisComponentInstClass(BaseElementClass):
  def __init__(self,name,type,parametersDict,hvlParametersDict,extDef,instanceArraySize=''):
    ...
    for parameter in hvlParametersDict:
      self.hvl_parameters.append(ParameterValueClass(parameter['name'],parameter['value']))

class envScoreboardClass(BaseElementClass):
  def __init__(self,name,sType,tType,parametersDict,instanceArraySize=''):
...
class connectionClass(BaseElementClass):
  def __init__(self,name,pName,subscriberName, aeName, validate, loopIter):
...
class EnvironmentClass(BaseGeneratorClass):
  ...
  def __init__(self,name):
    ...
    self.mtlbReady = False
    self.dest_dir_override = None
    self.envHasComponentArrays = False
  ...
  def initTemplateVars(self,template):
    ...
    template['external_imports'] = self.external_imports
    template['paramDefs'] = self.paramDefs
    template['hvlParamDefs'] = self.hvlParamDefs
    template['configVariableValues'] = self.configVariableValues
    template['hvlPkgParamDefs'] = self.hvlPkgParamDefs
    ...
    template['svLibNames'] = self.svLibNames
    template['mtlbReady'] = self.mtlbReady
    template['envHasComponentArrays'] = self.envHasComponentArrays
    return template
  ...
  def addNonUvmfComponent(self,name,type, parametersDict={},instanceArraySize=''):
    """Add an agent instantiation to the definition of this environment class"""
    self.nonUvmfComponents.append(NonUvmfComponentClass(name,type,parametersDict,instanceArraySize))
    if ( instanceArraySize != ''):
      self.envHasComponentArrays = True
  ...
  def addAgent(self,name,ifPkg,clk,rst,parametersDict={},initResp='INITIATOR',instanceArraySize=''):
    """Add an agent instantiation to the definition of this environment class"""
    self.agents.append(AgentClass(name,ifPkg,clk,rst,self.agentIndex,parametersDict,initResp,instanceArraySize))
  ...
  def addSubEnv(self,name,envPkg,numAgents,parametersDict={}, regModelPkg=None,regBlockClass=None,regBlockInstance='',instanceArraySize=''):
    if ( regBlockInstance == ''):
      regBlkInst = name+"_rm"
    else:
      regBlkInst = regBlockInstance
    """Add a sub environment instantiation to the definition of this environment class"""
    self.subEnvironments.append(SubEnvironmentClass(name,envPkg,numAgents,self.agentIndex,parametersDict,regModelPkg,regBlockClass,regBlkInst,instanceArraySize))
  ...
  def addQvipSubEnv(self,name,envPkg,agentList,envHasICVIP,envHasQVIP,instanceArraySize=''):
    """Add a sub environment instantiation to the definition of this environment class"""
    self.numAgents = agentList.__len__()
    self.qvipSubEnvironments.append(QvipSubEnvironmentClass(name,envPkg,self.numAgents,self.agentIndex,agentList,envHasICVIP,envHasQVIP,instanceArraySize))
  ...
  def addAnalysisPort(self,name,tType,connection="",portName="",instanceArraySize=None):
    """Build and connect an analysis port connection of the given name and transaction type"""
    self.analysis_ports.append(AnalysisPortClass(name,tType,connection,portName,instanceArraySize))
    if instanceArraySize:
      self.envHasComponentArrays = True

  def addAnalysisExport(self,name,tType,connection="",portName="",instanceArraySize=None):
    """Build and connect an analysis export connection of the given name and transaction type"""
    self.analysis_exports.append(AnalysisExportClass(name,tType,connection,portName,False,instanceArraySize))
    if instanceArraySize:
      self.envHasComponentArrays = True
  ...
  def defineAnalysisComponent(self,keyword,name,exportDict,portDict,qvipExportDict={},parametersList=[],hvlParametersList=[],mtlbReady=False):
    """Defines a type of analysis component for use later on."""
    ## Register the desired analysis component on the types array
    self.analysisComponentTypes.append(analysisComponentClass(keyword,name,exportDict,portDict,qvipExportDict,parametersList,hvlParametersList,mtlbReady))
  ...
  def addAnalysisComponent(self, name, pType, parametersList=[],hvlParametersList=[],extDef=False,instanceArraySize=''):
    """Add an analysis component instance  to the definition of this environment class"""
    self.analysisComponents.append(analysisComponentInstClass(name,pType,parametersList,hvlParametersList,extDef,instanceArraySize))

  # addUvmfScoreboard(instanceName, scoreboardType, transactionType)
  def  addUvmfScoreboard(self, name, sType, tType,parametersDict={},instanceArraySize=''):
    """Add scoreboard instance to the definition of this environment class"""
    self.scoreboards.append(envScoreboardClass(name,sType,tType,parametersDict,instanceArraySize))
    if ( instanceArraySize != ''):
      self.envHasComponentArrays = True

  # addConnection(outputComponentName, outputPortName, inputComponentName, inputPortName)
  def  addConnection(self, name, pName, subscriberName, aeName, validate=True, loopIter=None):
    """Add a connection between two components in the definition of this environment class"""
    self.connections.append(connectionClass(name,pName,subscriberName, aeName, validate, loopIter))
  ...
  def create(self,desired_template='all',parser=None,archive_yaml=True):
    ...
    for ac in self.analysisComponents:
      ac.hvl_parameters = [ParameterValueClass("CONFIG_T", "CONFIG_T")] +  ac.hvl_parameters
    ...
    for analysisComp in self.analysisComponentTypes:
      ...
      analysisComp.hvl_parameters = [ParamDef("CONFIG_T","type",None)] + [ParamDef("BASE_T","type","uvm_component")] + analysisComp.hvl_parameters
      self.runTemplate(analysisComp.keyword+".TMPL",analysisComp.keyword,{"name":analysisComp.name,
                                                     ...
                                                     "hvlParameters":analysisComp.hvl_parameters
                                                      })
...
class BenchClass(BaseGeneratorClass):
  ...
  def __init__(self,name,env_name,parametersDict={},hvlParametersDict={}):
    ...
    for parameterName in hvlParametersDict:
      self.envHvlParamDefs.append(ParameterValueClass(parameterName,hvlParametersDict[parameterName]))
    self.dest_dir_override = None
    self.BFMInstructions = []
    self.BFMGenvars = []
  ...
  def initTemplateVars(self,template):
    ...
    template['BFMInstructions'] = self.BFMInstructions
    template['BFMGenvars'] = self.BFMGenvars
  ...
  def addBfm(self,name,ifPkg,clk,rst,activity,parametersDict={},sub_env_path='environment',initResp='INITIATOR',vipLibEnvVariable='UVMF_VIP_LIBRARY_HOME',agentInstName='agent_inst_name',inFactReady=False,portList=[],instanceArraySize='',flat_bfm_name='',real_path=''):
    ...
    if '[' in real_path:
      is_arrayed = True
    self.bfms.append(BfmClass(name,ifPkg,clk,rst,activity,parametersDict,sub_env_path,initResp,agentInstName,inFactReady,portList,instanceArraySize,flat_bfm_name,is_arrayed,real_path))
  ...
  def addQvipBfm(self,name,ifPkg,activity,unique_id="",sequencer="",vipPkg=""):
